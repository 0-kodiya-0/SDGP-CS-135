import { FileMetadata, FileEntry, FileReadOptions } from './types';

/**
 * Implements an in-memory file system for temporary file storage
 */
export class MemoryFileSystem {
    ...
    /**
     * Stores file content directly in memory
     * @param name The filename
     * @param content The file content as ArrayBuffer
     * @param type The MIME type
     * @returns Promise resolving to the file metadata
     */
    async storeFileContent(name: string, content: ArrayBuffer, type: string): Promise<FileMetadata> {
        try {
            const metadata = this.createFileMetadataFromContent(name, content, type);
            ...
            throw new Error('Failed to store file content in memory');
        }
    }
