...
export class LocalForageProvider implements StorageProvider {
    ...
    async set<T>(key: string, value: T): Promise<void> {
        ...
        try {
            // First check if the value is the same as what's already stored
            const existingValue = await this.storage.getItem<T>(key);
            
            // Only proceed with setting if the value is different
            if (existingValue !== null && lodash.isEqual(existingValue, value)) {
                localforageLogger('Value unchanged, skipping write for key: %s', key);
                return;
            }
            
            // Only set the value if it's different or didn't exist
