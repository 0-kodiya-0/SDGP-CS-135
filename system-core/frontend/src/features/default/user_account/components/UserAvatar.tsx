...
export const UserAvatar: React.FC<UserAvatarProps> = ({
  ...
  const getProviderIcon = () => {
    if (!showProviderIcon || !account || !account.provider) return null;

    if (account.accountType === AccountType.Local) {
      return <KeyRound className="absolute -bottom-1 -right-1 bg-white rounded-full p-0.5 text-gray-700 shadow-sm border border-gray-200" size={iconSize} />;
    }
    ...
    switch (account.provider.toLowerCase()) {
      ...
      case 'microsoft':
        return (
          <div className="absolute -bottom-1 -right-1 bg-white rounded-full p-0.5 shadow-sm border border-gray-200">
            <svg viewBox="0 0 23 23" width={iconSize} height={iconSize}>
  ...
  // Handle missing or null account gracefully
  if (!account) {
    return (
      ...
      {hasValidImage ? (
        <img
          ...
          onError={(e) => {
            ...
            (e.target as HTMLImageElement).onerror = null; // Prevent infinite callbacks
          }}
        />
      ) : (
